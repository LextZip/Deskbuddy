...
// WIFI
// =========================================================
const char* WIFI_SSID = "Zumba-town";       // Replace with your WiFi network name
const char* WIFI_PASS = "isxyc31048";   // Replace with your WiFi password
...
String timezoneRule = "CET-1CEST,M3.5.0/2,M10.5.0/3";
...
// Sunrise / Sunset
static time_t sunriseEpoch = 0;
static time_t sunsetEpoch  = 0;
...
String sanitizeTimezoneRule(const String& value);
void applyTimezoneRule(const String& value);
...
static int minuteOfDayFromEpochLocal(time_t t) {
  if (t <= 0) return -1;
  struct tm tmLocal;
  localtime_r(&t, &tmLocal);
  return tmLocal.tm_hour * 60 + tmLocal.tm_min;
...
static String nextSunLabel() {
  ...
  int sunriseMin = minuteOfDayFromEpochLocal(sunriseEpoch);
  int sunsetMin = minuteOfDayFromEpochLocal(sunsetEpoch);
...
String sanitizeTimezoneRule(const String& value) {
  ...
  sanitized.trim();
  if (sanitized.length() == 0) return "CET-1CEST,M3.5.0/2,M10.5.0/3";
  if (sanitized.length() > 64) sanitized = sanitized.substring(0, 64);
  return sanitized;
}

void applyTimezoneRule(const String& value) {
  timezoneRule = sanitizeTimezoneRule(value);
...
void loadStoredSettings() {
  ...
  timezoneRule     = sanitizeTimezoneRule(prefs.getString("tz", timezoneRule));
  ...
  applyTimezoneRule(timezoneRule);
}
...
void ensureSunTimesForToday() {
  ...
  if ((sunriseEpoch <= 0 || sunsetEpoch <= 0 || ymd != lastSunYmd) &&
...
void drawClockCardSprite(bool force = false) {
  ...
  String sr = formatMinuteOfDay(minuteOfDayFromEpochLocal(sunriseEpoch));
  String ss = formatMinuteOfDay(minuteOfDayFromEpochLocal(sunsetEpoch));
...
void handleRoot() {
  ...
  String tzRule = sanitizeTimezoneRule(prefs.getString("tz", timezoneRule));
  ...
  page += "</select></div>";
  page += "<div style='grid-column:1 / -1;'><label class='label'>Timezone rule</label><input name='tz' maxlength='64' value='" + htmlEscape(tzRule) + "'><div class='muted' style='margin-top:8px;'>POSIX TZ string used by the clock, sunrise/sunset, and sync time. Example for Norway: CET-1CEST,M3.5.0/2,M10.5.0/3.</div></div>";
  page += "</div>";
...
void handleSave() {
  ...
  String newTimezoneRule = server.hasArg("tz") ? server.arg("tz") : timezoneRule;
  ...
  newLoc.trim();
  newNickname.trim();
  newTimezoneRule = sanitizeTimezoneRule(newTimezoneRule);
  ...
  prefs.putString("tz", timezoneRule);
